<?php 
    ...
    class Region{
        ...
        public function loadAllData(){
            $sql = "SELECT idDep,nombreDep,idPais FROM region";
            $stmt= self::$conn->prepare($sql);
            $stmt->execute();
        ...
        public function deleteData($id){
            $sql = "DELETE FROM region where idDep = :id";
            $stmt= self::$conn->prepare($sql);
            $stmt->bindParam(':id', $id);
            $stmt->execute();
        }
        public function updateData($data){
            $sql = "UPDATE region SET nombreDep = :nombreDep, idPais = :idPais where idDep = :idDep";
            $stmt= self::$conn->prepare($sql);
